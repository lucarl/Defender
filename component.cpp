...
void RenderComponent::Update(float dt) {
    if (!go->enabled)
        return;
    if (sprite) {
        sprite->draw(int(go->horizontalPosition), int(go->verticalPosition), (go->reverse));
    }
...
void CollideComponent::Update(float dt)
{
    for (auto i = 0; i < coll_objects->pool.size(); i++)
    {
        ...
        if (go0->enabled)
        {
            if ((go0->horizontalPosition > go->horizontalPosition - 25) &&
                (go0->horizontalPosition < go->horizontalPosition + 25) &&
